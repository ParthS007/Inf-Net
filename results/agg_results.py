...
if not valid_combinations:
    print("No combinations found with exactly 2 runs!")
    exit(1)

df_filtered = pd.concat(valid_combinations, ignore_index=True)
print(
    f"Found {len(df_filtered)} records with exactly 2 runs per combination ({len(df_filtered)//2} unique combinations)"
)

# Save filtered CSV (combined)
...
# Step 4: Calculate mean ± std deviation over 2 runs
print("\nStep 4: Calculating mean ± std deviation over 2 runs...")
...
if skipped_combinations > 0:
    print(f"Skipped {skipped_combinations} combinations (not exactly 2 runs)")
