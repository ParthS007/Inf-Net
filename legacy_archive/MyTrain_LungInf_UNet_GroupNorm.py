...
"""Preview
Code for UNet training on COVID-19 Lung Infection Segmentation from CT Scans
...
def train(train_loader, model, optimizer, epoch, save_path, opt, total_step):
    model.train()
    ...
    if (epoch + 1) % 10 == 0:
        checkpoint_path = os.path.join(save_path, f"UNet-{epoch+1}.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="if True, you will turn on the mode of `Semi-UNet`",
    )
    parser.add_argument(
        "--is_pseudo",
        type=bool,
    ...
    # UNet doesn't have pre-trained weights for COVID dataset
    ...
    for epoch in range(1, opt.epoch + 1):
        ...
        train(train_loader, model, optimizer, epoch, save_path, opt, total_step)
